...
module.exports = (_this, req, res, next) => {
    ...
    try {
        verified = jwt.verify(authHeader.token, process.env.SECRET || 'change this');
    } catch (err) {
        verified = 'Unable to verify token.';
  
    }
    if (verified === 'Unable to verify token.')  return reqMessage(req, 'Unable to verify token.', next);
